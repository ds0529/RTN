...
hdf5_data_dir = FLAGS.data_path+'/shapenet_hdf5_data'
ply_data_dir = FLAGS.data_path+'/shapenet_PartAnnotation'
gpu_to_use = 0
...
def pc_augment_to_point_num_2(pts, seg, pn):
    if pts.shape[0] <= pn:
        cur_len = pts.shape[0]
        res_pts = np.array(pts)
        res_seg = np.array(seg)
        while cur_len < pn:
            res_pts = np.concatenate((res_pts, pts))
            res_seg = np.concatenate((res_seg, seg))
            cur_len += pts.shape[0]
    else:
        res_pts = np.array(pts)
        res_seg = np.array(seg)

    return res_pts[:pn, :], res_seg[:pn]
...
def predict():
  ...
  with tf.device('/gpu:'+str(gpu_to_use)):
    ...
    # transformed points
    transformed_points = tf.matmul(pointclouds_ph, end_points['xyz_transform'])
    transformed_points_norm = tf.reduce_max(
        tf.sqrt(tf.reduce_sum(transformed_points * transformed_points, axis=-1)), axis=-1)
    transformed_points = transformed_points / tf.reshape(transformed_points_norm, (-1, 1, 1))
  ...
  with tf.Session(config=config) as sess:
    ...
    for shape_idx in range(len_pts_files):
      ...
      if not_origin:
          batch_data[0, ...], seg = pc_augment_to_point_num_2(pc_normalize(pts), seg, point_num)
      else:
          batch_data[0, ...] = pc_augment_to_point_num(pc_normalize(pts), point_num)

      batch_data, _, _ = provider.rotate_point_cloud_by_so3(batch_data)
      # batch_data = provider.rotate_point_cloud(batch_data)
      # batch_data = batch_data

      feed_dict = {pointclouds_origin_pl: batch_data[:, :NUM_POINT, :],
                   is_training_transformer_pl: is_training_transformer}
      pred_rotation_class_np = sess.run(pred_rotation_class, feed_dict=feed_dict)
      transformed_points_temp = provider.rotate_data_to_origin_classification_so3(batch_data,
                                                                            rotation_matrics_all,
                                                                            np.argmax(pred_rotation_class_np,
                                                                                      axis=1))

      seg_pred_res, transformed_points_np = sess.run([seg_pred, transformed_points], feed_dict={
            pointclouds_ph: transformed_points_temp,
      ...
      if not_origin:
          seg_pred_val = np.argmax(seg_pred_res, axis=1)
      else:
