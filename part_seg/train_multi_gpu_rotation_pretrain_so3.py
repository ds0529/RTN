...
parser.add_argument('--batch', type=int, default=12, help='Batch Size per GPU during training [default: 32]')
      ...
      for i in range(FLAGS.num_gpu):
        with tf.device('/gpu:%d' % i):
          with tf.name_scope('%s_%d' % (TOWER_NAME, i)) as scope:
            ...
            seg_pred, _ = model.get_model(pointclouds_phs[-1], input_label_phs[-1], \
    ...
    def train_one_epoch(train_file_idx, epoch_num):
      ...
      for i in range(num_train_file):
        ...
        for j in range(num_batch):
          ...
          batch_data0, _, _ = provider.rotate_point_cloud_by_so3(cur_data[begidx_0: endidx_0, ...])
          feed_dict = {pointclouds_origin_pl: batch_data0[:, :NUM_POINT, :],
          ...
          transformed_points0 = provider.rotate_data_to_origin_classification_so3(batch_data0,
                                                                               rotation_matrics_all,
                                                                               np.argmax(pred_rotation_class_np, axis=1))


          batch_data1, _, _ = provider.rotate_point_cloud_by_so3(cur_data[begidx_1: endidx_1, ...])
          feed_dict = {pointclouds_origin_pl: batch_data1[:, :NUM_POINT, :],
                       is_training_transformer_pl: is_training_transformer}
          pred_rotation_class_np = sess.run(pred_rotation_class, feed_dict=feed_dict)
          transformed_points1 = provider.rotate_data_to_origin_classification_so3(batch_data1,
                                                                                rotation_matrics_all,
    ...
    def eval_one_epoch(epoch_num):
      ...
      for i in range(num_test_file):
        ...
        for j in range(num_batch):
          ...
          batch_data, _, _ = provider.rotate_point_cloud_by_so3(cur_data[begidx: endidx, ...])
          feed_dict = {pointclouds_origin_pl: batch_data[:, :NUM_POINT, :],
                       is_training_transformer_pl: is_training_transformer}
          pred_rotation_class_np = sess.run(pred_rotation_class, feed_dict=feed_dict)
          transformed_points = provider.rotate_data_to_origin_classification_so3(batch_data,
                                                                                rotation_matrics_all,
                                                                                np.argmax(pred_rotation_class_np,
                                                                                          axis=1))

          feed_dict = {
              pointclouds_phs[1]: transformed_points,
