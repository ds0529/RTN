...
parser.add_argument('--log_dir', default='result/log_learning_rotation_mix_model_so3_30_30_1000', help='Log dir [default: log]')
...
TRAIN_FILES = provider.getDataFiles( \
    os.path.join(BASE_DIR, '/mnt/dengshuang/data/modelnet40/aligned_modelnet40_ply_hdf5_2048/train_files.txt'))
TEST_FILES = provider.getDataFiles(\
    os.path.join(BASE_DIR, '/mnt/dengshuang/data/modelnet40/aligned_modelnet40_ply_hdf5_2048/test_files.txt'))
...
ply_dict = {}
ply_dict['save_ply_dir'] = 'result/origin_ply/'
...
def create_output(points_all):
    shape_num = points_all.shape[0]
    for i in range(shape_num):
        point_num = points_all.shape[1]
        colors = np.tile(np.array([0, 1, 0], np.int)*255, [point_num, 1])

        points = points_all[i]+np.array([ply_dict['row_index']*ply_dict['gap'], ply_dict['col_index']*ply_dict['gap'], 0])
